...
class AlignmentV3Model(nn.Module):
    ...
    def image_pre_projection(self, images: torch.Tensor) -> torch.Tensor:
        """The representation handed to ``image_projection``.

        Exposed for the queue identification control, which stores this
        instead of the projected embedding so that queued negatives can be
        re-projected through the *current* projector.  For configurations
        with no token aggregator and no adapter (the queue factorial student)
        this is a purely frozen encoder feature, so re-projection removes
        staleness completely.  When an aggregator or adapter is present those
        modules are trainable and their staleness is not removed; see
        ``queue_identification`` for the guard that rejects that case.
        """
    ...
    def encode_image(self, images: torch.Tensor) -> torch.Tensor:
        return F.normalize(self.image_projection(self.image_pre_projection(images)), dim=-1)

    def encode_text(self, captions: list[str]) -> torch.Tensor:
        return self.encode_text_tokens(self.text_encoder.tokenize(captions))

    def text_pre_projection_tokens(self, tokens: object) -> torch.Tensor:
        """The representation handed to ``text_projection``.  See
        :meth:`image_pre_projection` for why this is exposed."""
        if self.text_token_aggregator is None:
            return self.text_encoder.forward_tokens(tokens)
        ...
        return self.text_token_aggregator(token_states, attention_mask, baseline)

    def text_pre_projection(self, captions: list[str]) -> torch.Tensor:
        return self.text_pre_projection_tokens(self.text_encoder.tokenize(captions))

    def encode_text_tokens(self, tokens: object) -> torch.Tensor:
        return F.normalize(
            self.text_projection(self.text_pre_projection_tokens(tokens)), dim=-1
        )

    def forward(self, images: torch.Tensor, captions: list[str]) -> dict[str, torch.Tensor]:
        image_pre = self.image_pre_projection(images)
        image_embeds = F.normalize(self.image_projection(image_pre), dim=-1)
        text_pre = self.text_pre_projection(captions)
        text_embeds = F.normalize(self.text_projection(text_pre), dim=-1)
        scale = self.logit_scale.exp().clamp(max=100.0)
        output = {
            "image_embeds": image_embeds,
            "text_embeds": text_embeds,
            "image_pre_projection": image_pre,
            "text_pre_projection": text_pre,
