...
# images may live outside the repo; point ALIGNMENT_DATA_ROOT at that tree if so
DATA_FALLBACK_ROOT = Path(os.environ.get("ALIGNMENT_DATA_ROOT", str(ROOT)))
    ...
    with source_csv.open(newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
            ...
            if not source.is_absolute():
                candidates.append(DATA_FALLBACK_ROOT / source)
